...
<%
	try{
		...
		ArrayList<String> jsonListString = (ArrayList<String>)session.getAttribute("jsonListString");
		String jsonString = "";	
		//Extracts the Strigns from the ArrayList
		for(int j = 0; j < jsonListString.size(); j++){
			...
			jsonString = jsonListString.get(j);
						
			if(!jsonString.equals("") && !jsonString.equals("[]")){
				
				JSONArray json = new JSONArray(jsonString);
				
				//Extracts data from json, since the data is cumulative, takes only the last day of each month
				...
				//Prepares the picture and render it on the page
				try {
					...
					final ChartRenderingInfo info = new ChartRenderingInfo(new StandardEntityCollection());
		         	final File file1 = new File(getServletContext().getRealPath(".") + "/" + j + ".png");
		         	ChartUtilities.saveChartAsPNG(file1, fail, 700, 600, info);
		         	%><img src="<%=j%>.png" WIDTH='700' HEIGHT='600' BORDER='0'><%
			}
		}
